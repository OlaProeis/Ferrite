...
Ready on `0.3.1-experimental`; not yet tagged.

### Added

#### Platform verification (carry-over gates)
- **v0.3.0 regression matrix re-run** — Documented outcomes for carry-over release gates in [`v0.3.0-regression-matrix.md`](docs/technical/platform/v0.3.0-regression-matrix.md). **Windows borderless ([#112](https://github.com/OlaProeis/Ferrite/issues/112)):** WIN-1…WIN-7 pass on Win11 with egui 0.34.2; Intel-iGPU WIN-8 row still blank (needs dedicated hardware). **Ubuntu Wayland keyboard ([#106](https://github.com/OlaProeis/Ferrite/issues/106), KBD-8):** not yet verified on Ubuntu 24.04 Wayland — issue stays open; `WAYLAND_DISPLAY= ferrite` workaround remains documented for affected builds. **macOS Sonoma keyboard ([#111](https://github.com/OlaProeis/Ferrite/issues/111), KBD-9):** not yet verified on Sonoma hardware — issue stays open.

#### Video embeds ([#119](https://github.com/OlaProeis/Ferrite/issues/119))
- **Video embed parsing** — Explicit `{{video URL}}` syntax → `VideoEmbed` AST node with allowlist and round-trip `source_text`. See [`video-embed-parsing.md`](docs/technical/markdown/video-embed-parsing.md).
- **Trusted YouTube playback** — wry child WebView overlay on the rendered preview rect for allowlisted hosts. The WebView loads a local **relay page** (`ferrite-video://localhost/embed?v=…`) that hosts a `youtube-nocookie.com` iframe with a valid embed origin — YouTube rejects top-level `/embed/` navigation (Error 153). Mandatory thumbnail + play-overlay fallback when WebView creation/positioning fails or the embed scrolls off-screen. Untrusted hosts never receive a WebView. See [`video-embeds.md`](docs/technical/markdown/video-embeds.md), [`video-embed-rendering.md`](docs/technical/markdown/video-embed-rendering.md).

#### Mermaid — second wave
- **Git graph rewrite** — Full core Mermaid `gitGraph` grammar (tags, branch types, cherry-pick, LR/TB), horizontal lane layout, and native egui rendering. See [`git-graph-parser.md`](docs/technical/mermaid/git-graph-parser.md), [`git-graph-layout.md`](docs/technical/mermaid/git-graph-layout.md), [`git-graph-render.md`](docs/technical/mermaid/git-graph-render.md).
- **Flowchart `@pos` manual layout** — Parse `%% @pos <node_id> <x> <y>` hints; apply pixel overrides after Sugiyama layout. See [`flowchart-pos-hints.md`](docs/technical/mermaid/flowchart-pos-hints.md), [`manual-layout.md`](docs/technical/mermaid/manual-layout.md).
- **Flowchart FC-83b** — Strip leading `fa:fa-*` / `fab:fa-*` icon prefixes from node and edge labels (glyph not rendered). See [`flowchart-fa-labels.md`](docs/technical/mermaid/flowchart-fa-labels.md).
- **Flowchart `linkStyle interpolate basis`** — Catmull-Rom smooth curves on routed edge paths. See [`flowchart-linkstyle-interpolate.md`](docs/technical/mermaid/flowchart-linkstyle-interpolate.md).
- **mmdr evaluation spike** — Decision doc for `mermaid-rs-renderer` parser reuse: **partial-adopt** (parser frontend only; keep native egui rendering). See [`mmdr-evaluation.md`](docs/technical/mermaid/mmdr-evaluation.md).

#### Multi-window ([#125](https://github.com/OlaProeis/Ferrite/issues/125))
- **New Window** — Second OS viewport with independent tab strip via `show_viewport_immediate`. Design: [`multi-window.md`](docs/technical/platform/multi-window.md); MVP: [`multi-window-implementation.md`](docs/technical/platform/multi-window-implementation.md).
- **Window-aware file opens** — Opens route to the focused document window; single-instance IPC forwards paths to the last-focused window. See [`multi-window-file-routing.md`](docs/technical/platform/multi-window-file-routing.md), [`single-instance.md`](docs/technical/platform/single-instance.md).
- **Session schema v2** — Multi-window geometry and per-window tab sets restore on startup; v1 sessions deserialize as a single window. See [`multi-window-session.md`](docs/technical/platform/multi-window-session.md).

#### GitHub HTML subset (Phases 1–2)
- **Phase 1 blocks** — `<div align>`, `<details>/<summary>`, `<br>` with sanitization and rendered collapse. See [`github-html-block-subset.md`](docs/technical/markdown/github-html-block-subset.md).
- **Phase 2 inline** — `<kbd>`, `<sup>`, `<sub>`, sized `<img width/height>`. Full tag list: [`github-html-subset.md`](docs/technical/markdown/github-html-subset.md).

#### Tables & data viewers
- **GFM table column alignment ([#140](https://github.com/OlaProeis/Ferrite/issues/140))** — Rendered per-column left/center/right layout, toolbar alignment cycling, delimiter round-trip through rendered edit sessions. See [`gfm-table-column-alignment.md`](docs/technical/markdown/gfm-table-column-alignment.md).
- **Raw-mode GFM table column guides** — Display-only vertical pipe guides in the raw markdown editor; blake3 cache per table block; no source mutation. See [`raw-table-alignment.md`](docs/technical/editor/raw-table-alignment.md).
- **CSV rendered cell editing (MVP)** — Double-click to edit cells in the CSV/TSV Rendered view for small files; RFC 4180 serialization, undo integration, keyboard commit/cancel. Large lazy-parsed files still prompt to edit in Raw view. See [`csv-viewer.md`](docs/technical/viewers/csv-viewer.md).

#### Editor & preview UX
- **Tab strip context menu** ([#150](https://github.com/OlaProeis/Ferrite/pull/150)) — Right-click tab menu (New Tab, Close Tab, Copy Path, Reveal in Explorer); click vs drag sense split so context menu stays responsive during drag-reorder; file-path hover tooltip. Metadata-only `ActionRegistry` in `src/ui/action_registry.rs` (app owns side effects). Thanks [@Star-sumi](https://github.com/Star-sumi).
- **Outline tree visual polish** ([#151](https://github.com/OlaProeis/Ferrite/pull/151)) — Tree guide lines, node bullets, pill-style active highlight, semantic per-heading-type colors, richer item tooltip (`outline.item_tooltip`). Thanks [@Star-sumi](https://github.com/Star-sumi). See [`outline-panel.md`](docs/technical/ui/outline-panel.md).
- **Preview lock ([#144](https://github.com/OlaProeis/Ferrite/issues/144))** — Per-tab padlock overlay on preview panes; blocks WYSIWYG, CSV, and Tree mutations while the raw/split-left pane stays editable; persists across restart. See [`preview-lock.md`](docs/technical/ui/preview-lock.md), [`preview-lock-mode.md`](docs/technical/markdown/preview-lock-mode.md).
- **Word wrap toggle ([#145](https://github.com/OlaProeis/Ferrite/issues/145))** — **Alt+Z** / **Option+Z** flips persisted `settings.word_wrap`; command palette entry; toast when the active tab is in uniform-height mode (100K+ lines). See [`word-wrap.md`](docs/technical/editor/word-wrap.md) § User Toggle, [`keyboard-shortcuts.md`](docs/technical/ui/keyboard-shortcuts.md).
- **Rendered click-to-edit cursor precision** — Shared `FormattedBlockLayout` galley for paint + hit-test; per-block `layout_wrap_width` persistence. See [`rendered-edit-session-formatted-layout.md`](docs/technical/markdown/rendered-edit-session-formatted-layout.md).
- **External file open fallback ([#142](https://github.com/OlaProeis/Ferrite/issues/142))** — Binary/denylisted files open in the OS default app via `open::that` with a toast (`OpenResult`). See [`external-file-open-fallback.md`](docs/technical/files/external-file-open-fallback.md).

#### Workspace & runtime visibility
- **File tree polish ([#135](https://github.com/OlaProeis/Ferrite/issues/135))** — Hover highlight and active-tab row emphasis in the workspace sidebar. See [`file-tree-panel.md`](docs/technical/ui/file-tree-panel.md).
- **Stats runtime modules (Phase 1)** — Read-only section on the Stats tab: loaded CJK/complex fonts, Mermaid cache size, terminal sessions; LSP row shows **Disabled**. See [`stats-runtime-modules.md`](docs/technical/ui/stats-runtime-modules.md).

#### Code block Run — hardening
- **Shell interpreter dispatch** — Windows `bash`/`shell` without Git Bash fails fast with a clear message; POSIX fences never dispatch to PowerShell on Windows; `sh`/`zsh` fallback chain documented and implemented. See [`code-block-run.md`](docs/technical/markdown/code-block-run.md) § Shell interpreter dispatch.
- **Stable run-state identity** — Inline output keyed by blake3 hash of language + code (survives edits above the fence).
- **Running-with-no-output UX** — "Waiting for output…" placeholder while `RunStatus::Running` with empty streams.
- **Copy / Insert stderr labelling** — Clipboard and fenced insert prefix stderr to match the on-screen panel.

#### Platform & distribution (Tier C)
- **Optional native title bar ([#115](https://github.com/OlaProeis/Ferrite/issues/115))** — Settings → Appearance: **Use system title bar** on Linux/macOS (default off; restart required). Windows: disabled with tooltip. See [`system-title-bar-setting.md`](docs/technical/platform/system-title-bar-setting.md).
- **Optional Windows Inno Setup installer** — `installer/ferrite.iss` builds unsigned `ferrite-windows-x64-setup.exe`; MSI remains the recommended signed install. See [`inno-setup-installer.md`](docs/technical/platform/inno-setup-installer.md).

### Fixed

- **YouTube inline embed Error 153 and WebView focus steal** ([#119](https://github.com/OlaProeis/Ferrite/issues/119)) — Trusted YouTube embeds no longer navigate the child WebView directly to `youtube.com/embed/…` (YouTube returns **Error 153: Video player configuration error** for top-level embed URLs). Playback now uses a wry custom-protocol relay page with an iframe, `referrerpolicy="strict-origin-when-cross-origin"`, and `origin=location.origin`. Child WebViews are created with `with_focused(false)`; `focus_parent()` runs on teardown and each frame when the pointer is outside the embed (or over the split raw pane / title bar) so WebView2 does not keep HWND focus and lock window drag or raw editing while a video plays. Native WebViews are hidden (`set_visible(false)`) only where a foreground egui overlay rect intersects the embed (unsaved-changes dialog, quick switcher, command palette, find/replace, etc.) so modals paint above the player without removing video in non-overlapping regions. Fixed the occlusion pass never firing: `end_frame()` cleared the per-frame embed-rect map before `apply_foreground_occlusion()` ran (overlays render after the markdown pass), so every intersection test saw an empty map and the WebView was unconditionally re-shown each frame — embed rects now persist until the next `begin_frame()`. The embed WebView is also hardened with a navigation allowlist (relay page + `youtube-nocookie.com` player only; everything else blocked and logged) and a new-window handler that denies popups and routes "Watch on YouTube"-style links to the system browser. **Multi-window:** opening a second document window no longer destroys the primary window's video WebView every frame (the player vanished while a second window was open) — secondary viewport passes shared `render_central_panel()` and tore down the WebViews via `end_frame`/`clear_all()`; inline WebView playback is now primary-window-only (`ViewportId::ROOT` gate), secondary windows render embeds via the thumbnail → system-browser fallback, and occluder rects from secondary viewports are ignored. Failed WebView creates are blacklisted per embed key to avoid per-frame recreate stalls. See [`video-embeds.md`](docs/technical/markdown/video-embeds.md) § Multi-window. See [`video-embeds.md`](docs/technical/markdown/video-embeds.md) § Relay page / Focus handling / Modal z-order.
- **GitHub HTML subset — parser & rendered view** — Post-ship fixes for Phases 1–2 (manual fixture: [`test_md/test_github_html.md`](test_md/test_github_html.md)). **Parser:** `coalesce_inline_wrapper` no longer indexes a stale close tag after `drain()` (crash on `<kbd>` chords such as `Ctrl+Shift+P`). **Parsing:** single-line `<div align="…">…</div>` blocks coalesce into `AlignedDiv` instead of `«HTML»` passthrough. **Rendering:** paragraphs with `<kbd>`, `<sup>`, `<sub>`, or inline `<br>` use AST inline rendering (not raw source text); rich `<summary>` HTML (e.g. `<strong>`) renders as formatted inline text; center alignment positions shrink-wrapped blocks via layout justify (avoids galley `halign` bleeding into the split-view divider). See [`github-html-subset.md`](docs/technical/markdown/github-html-subset.md), [`github-html-block-subset.md`](docs/technical/markdown/github-html-block-subset.md).
...
- **Test build baseline** ([#152](https://github.com/OlaProeis/Ferrite/pull/152)) — `#[cfg(test)]` imports for history undo tests, code-execution status glyphs, and flowchart parser helpers; back-edge layout test uses shared `expand_rect`. Thanks [@Star-sumi](https://github.com/Star-sumi).
- **Exit / close "Don't Save" resurrected discarded changes on restart** — Shutdown ordering bug: answering **Don't Save** in the exit dialog could still leave recovery JSON and `has_unsaved_content: true` in `session.json` because `on_exit()` ran afterwards and unconditionally re-persisted every still-modified tab. On the next launch the identity gate passed (same path, unchanged disk) and the **Recovered content differs from this file on disk** banner — plus the autosave restore dialog when reopening the file — offered changes the user had explicitly thrown away. **Fix:** `discard_unsaved_on_exit` defers cleanup to `on_exit`, which skips only the prompted tab set via `save_recovery_content_excluding()`, deletes their `recovery/<id>.json` and autosave temps, and forces `has_unsaved_content: false` in the captured session. Quick-note scratch buffers (non-prompting pathless tabs) keep their recovery content. Crash-recovery path unchanged. See [`session-persistence.md`](docs/technical/files/session-persistence.md) § On Exit with "Don't Save".
- **Exit / window-close "Save" only saved the active tab** — With two or more modified tabs the exit confirmation could never complete (**Save** re-saved the same tab forever). `handle_save_all_modified_tabs()` now saves every tab that triggered the prompt; pathless tabs are activated and routed through Save As one at a time; a failed or cancelled save keeps the dialog open.
- **Window-close "Save" did nothing** — `PendingAction::CloseWindow` had no Save branch; it now saves that window's modified tabs and closes when clean.
- **Tab / window "Don't Save" left autosave backups behind** — Discarded changes could reappear via the auto-save restore dialog on reopen; autosave temp files are now deleted alongside recovery files when the user confirms discard (tab close, window close, and exit).
- **Multi-window unsaved-dialog cleanup targeted wrong tabs** — Close-tab confirmation collected the tab id via the global store index instead of the window strip index; window-close confirmations did not clean per-tab recovery or UI state. Both fixed.
- **Windows 11 custom title bar: double-click restore disabled move/resize** ([#153](https://github.com/OlaProeis/Ferrite/issues/153)) — Restoring from maximized via title-bar double-click could leave the window stuck (no drag, no edge resize) until restart; the restore button and Win+↓ were unaffected. `StartDrag` no longer fires on double-click frames or while maximized. Reported on Windows 11; not reproduced on Windows 10. See [`custom-title-bar.md`](docs/technical/platform/custom-title-bar.md).

### Deferred

- **LSP integration (entire epic)** → **v0.3.2+** — All phases (transport hardening, problems panel, hover, go-to-def, completion, `lsp` feature-flag removal) deferred from v0.3.1 to prioritize Mermaid wave 2 and multi-window. Code remains in-tree behind the `lsp` Cargo feature flag; Stats panel shows LSP as **Disabled**. Planning docs unchanged; [`lsp-problems-panel.md`](docs/technical/lsp/lsp-inline-diagnostics.md) not added.
- **Word wrap toolbar/ribbon icon ([#145](https://github.com/OlaProeis/Ferrite/issues/145))** — Tier C; Alt+Z and command palette ship instead.
- **CSV Tab / Shift+Tab between cells** — Tier C follow-up → v0.3.2 (MVP cell editing shipped).
- **Stats panel Phase 2** (manual font unload, clear Mermaid cache) — Tier C → v0.3.2.
- **Mermaid drag-to-reposition** with `%% @pos` write-back — Tier C → v0.3.2 (`@pos` parse + layout ship).
- **File tree context menu: Open with system default ([#142](https://github.com/OlaProeis/Ferrite/issues/142))** — Tier C → v0.3.2 (automatic fallback ships).
- **GitHub HTML Phase 3** (nested HTML, HTML tables) — v0.3.2 per PRD.
- **Platform gates [#106](https://github.com/OlaProeis/Ferrite/issues/106) / [#111](https://github.com/OlaProeis/Ferrite/issues/111)** — KBD-8/KBD-9 matrix rows still open; community verification welcome.
...
- **Video embeds: explicit `{{video}}` syntax required** ([#119](https://github.com/OlaProeis/Ferrite/issues/119)) — Bare YouTube/youtu.be URLs on their own line no longer auto-embed; they render as normal clickable autolinks. Use `{{video URL}}` for inline playback. Existing docs with bare links need the braced syntax to keep the player. See [`video-embed-parsing.md`](docs/technical/markdown/video-embed-parsing.md).
